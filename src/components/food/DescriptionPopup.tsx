    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="w-[90%] rounded-lg">
        ...
        <DialogFooter>
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${item.food_type == "isVeg" ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              <Image
                src={item.food_type == "isVeg" ? '/veg.png' : '/nonveg.png'}
                width={20}
                height={20}
                alt={item.food_type == "isVeg" ? 'Vegetarian' : 'Non-vegetarian'}
              />
              {item.food_type == "isVeg" ? "Vegetarian" : "Non-Vegetarian"}
            </div>
